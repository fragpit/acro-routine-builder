...
- `scoring/` computes per-run technicity, AWT vs AWQ bonus formulas, and `eligibility.ts` (used by the palette to mark tricks as ignored with a reason when structural rules would forbid them). `scoring/technical-marks.ts` is the canonical place for per-manoeuvre technical mark defaults, clamping and run averaging; do not duplicate that math in UI code.
- `io/` handles program import/export: `program-json.ts` (JSON roundtrip) and `program-markdown.ts` (human-readable report). `Program.technicalMarksByManoeuvreId` is part of the JSON contract and stores custom marks by manoeuvre id, so imports, snapshots and sanitizers must preserve/default it alongside runs, notes and default bonuses. `io/download.ts` exposes `safeFileName` / `download` helpers reused by desktop and mobile file controls.
- `io/awt-api.ts` reads the AWT competition snapshot from `public/awt-snapshot/` (NOT api.acroworldtour.com). The snapshot is generated by `scripts/snapshot-awt.mjs` (run via `npm run snapshot:awt`) and committed to the repo. The script skips non-`closed` competitions - we only freeze immutable results. To pick up new events, run the script and open a PR. `io/awt-mapping.ts` converts the AWT JSON shape into our `Program` model; types in `awt-api.ts` mirror the AWT API response so the snapshot stores raw API payloads verbatim.
...
- `TrickInfoCard` is a side panel (not a popover) opened on cell click; it shows the trick description, bonus checkboxes with mutual-exclusion disabling, and the custom technical mark control shared by desktop and mobile.
...
- In `docs/news.md`, use a short descriptive English `##` heading that explains
  the change. Never use a version-only heading such as `## v0.9.12`.
- Keep the newest entry first because the unread-news logic treats the first
  `##` section as the latest item.
- Track release versions in `CHANGELOG.md`, not in News headings.
