...
- **bonusPercent** - raw sum of selected bonuses on every trick in
  the run.
- **scaledBonusPercent** - selected bonuses weighted by each trick's
  technical mark.
...
On mobile, the `Avg T` label stays together on the first line and
its percentage appears below it when space is tight.
...
increments. Both fields accept whole percentages only. Leave the
